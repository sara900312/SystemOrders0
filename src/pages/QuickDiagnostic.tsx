  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      try {
        ...
        addResult('حالة المصادقة', 'error', `خطأ: ${getErrorMessage(error, 'خطأ غير معروف')}`);
      }
      ...
      try {
        ...
        addResult('جدول المتاجر', 'error', `خطأ: ${getErrorMessage(error, 'خطأ في جدول المتاجر')}`);
      }
      ...
      try {
        ...
        addResult('دالة get_orders_with_products', 'error', `خطأ: ${getErrorMessage(error, 'خطأ في دالة الطلبات')}`);
      }
      ...
      try {
        ...
        addResult('جدول الإعدادات', 'error', `خطأ: ${getErrorMessage(error, 'خطأ في جدول الإعدادات')}`);
      }
      ...
      try {
        ...
        addResult('دالة admin-login-v2', 'success', 'الدالة تعمل (رفضت بيانات خاطئ��)', data);
      } catch (error) {
        addResult('دالة admin-login-v2', 'error', `خطأ: ${getErrorMessage(error, 'خطأ في دالة تسجيل الدخول')}`);
      }

    } catch (error) {
      addResult('خطأ عام', 'error', `خطأ غير متوقع: ${getErrorMessage(error, 'خطأ غير معروف')}`);
    } finally {
      setIsRunning(false);
    }
  };
