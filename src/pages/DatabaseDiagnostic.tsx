...
import { fullDatabaseFix, testOrderStatusUpdate } from '@/utils/databaseFix';
...
const DatabaseDiagnostic = () => {
  ...
  const testStoreResponse = async () => {
    const result = await testOrderStatusUpdate();

    if (result.success) {
      toast({
        title: 'نجح الاختبار',
        description: 'تم اختبار تحديث حالة الطلب بنجاح',
      });
    } else {
      toast({
        title: 'فشل الاختبار',
  ...
  const runDatabaseFix = async () => {
    ...
    const result = await fullDatabaseFix();
    ...
    <div className="p-6 max-w-6xl mx-auto" dir="rtl">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          فحص حالة الجداول وإصلاح مشاكل استجا��ة المتجر
        </p>
      </div>
