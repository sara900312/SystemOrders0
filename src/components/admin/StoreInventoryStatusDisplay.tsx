...
export function StoreInventoryStatusDisplay({
  ...
  // إذا كان الطلب مرفوض من الزبون، لا نعرض معلومات المتجر
  if (order.order_status === 'customer_rejected' || order.store_response_status === 'customer_rejected') {
    return null;
  }
  ...
  const statusConfig = isAvailable 
    ? {
        ...
        label: `✅ ${t('in.stock.full')}`,
        description: t('store.confirmed.availability')
      }
    : {
        icon: XCircle,
      ...
      <div className="flex-1">
        {showTitle && (
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-semibold text-blue-800">{t('inventory.check.title')}</span>
          </div>
        )}
        
        <div className="flex items-center gap-2">
          <span className="font-medium">{t('availability.status.label')}</span>
