...
export const useEnhancedEdgeFunctions = () => {
  ...
  const getOrderDetails = useCallback(async (orderId: string, storeId?: string): Promise<OrderDetailResponse | null> => {
    ...
    try {
      ...
      console.error('❌ خطأ في useEnhancedEdgeFunctions.getOrderDetails - النوع:', typeof error);
      console.error('❌ خطأ في useEnhancedEdgeFunctions.getOrderDetails - المحتوى:', error);

      let errorMessage = 'فشل في جلب تفاصيل الطلب';
