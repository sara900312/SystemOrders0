...
class NotificationService {
  ...
  async sendNotification(data: NotificationData): Promise<boolean> {
    try {
      ...
      if (success) {
        console.log('✅ Notification sent via new Realtime system');
        return true;
      }

      // في حالة فشل النظام الجديد، استخدم الطريقة القديمة
      console.log('⚠️ Falling back to legacy Edge Function...');
  ...
  async createNotification(data: NotificationData): Promise<boolean> {
    try {
      ...
      if (error) {
        console.error('❌ Failed to create notification:', this.formatError(error));
        return false;
      }
      ...
      console.error('❌ Exception creating notification:', this.formatError(error));
      return false;
    }
  }
  ...
  async getUnreadNotificationsCount(userId: string): Promise<number> {
    try {
      ...
      console.error('📊 Exception in getUnreadNotificationsCount:', {
        error: error,
        formattedError: JSON.stringify(error, null, 2),
        operation: 'getUnreadNotificationsCount',
        userId,
  ...
  async getUnsentNotificationsCount(userId: string): Promise<number> {
    try {
      ...
      console.error('📊 Exception in getUnsentNotificationsCount:', {
        error: error,
        formattedError: JSON.stringify(error, null, 2),
  ...
  async testDatabaseConnection(): Promise<{success: boolean, error?: string, tableExists?: boolean}> {
    try {
      ...
      if (error) {
        console.error('🔗 Database connection test failed:', this.formatError(error));
        return {
          success: false,
      ...
      console.error('🔗 Database connection test exception:', this.formatError(error));
      return {
        success: false,
        error: error?.message || this.formatError(error),
        tableExists: false
  ...
  setupRealtimeSubscription(userId: string, onNotification: (notification: NotificationData) => void): () => void {
    const channel = supabase
      ...
      .subscribe((status) => {
        console.log(`📡 Notifications channel for user ${userId} status:`, status);

        if (status === 'SUBSCRIBED') {
          console.log(`✅ Successfully subscribed to notifications for user: ${userId}`);
        } else if (status === 'CHANNEL_ERROR') {
          console.error(`❌ خطأ في Real-time للمستخدم ${userId}: CHANNEL_ERROR`);
        } else if (status === 'TIMED_OUT') {
          console.warn(`⏰ انتهت مهلة الاتصال بـ Real-time للمستخدم ${userId}`);
        }
      });
