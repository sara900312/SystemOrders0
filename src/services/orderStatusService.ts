...
import { updateOriginalOrderBasedOnDivisions } from './divisionCompletionService';
...
export async function updateOrderStoreResponse({
  ...
  try {
    ...
    // تحويل الحالات الجدي��ة إلى القديمة للتوافق مع قاعدة البيانات
    let dbStatus = status;
    if (status === 'accepted') {
      dbStatus = 'available';
    ...
    if (error) {
      console.error('❌ خطأ في تحديث حالة الطلب:', {
        ...
        operation: 'updateOrderStoreResponse',
        orderId,
        storeId,
        status,
        rejectionReason,
    ...
    // للطلبات المقسمة: تحديث حالة الطلب الأصلي بناءً على حالة التقسيمات
    const orderDetails = data[0].order_details || '';
    if (orderDetails.includes('تم تقسيمه من الطلب الأصلي')) {
      // استخراج معرف الطلب الأصلي من التفاصيل
      ...
      if (match && match[1]) {
        ...
        // تحديث حالة الطلب الأصلي (بدون انتظار النتيجة لتجنب التأخير)
        updateOriginalOrderBasedOnDivisions(originalOrderId).catch(error => {
          // تسجيل الخطأ بمستوى منخفض لأنه لا يؤثر على عملية المتجر
          console.info('ℹ️ تحديث الطلب ال��صلي (خطأ غير حرج):', getErrorMessage(error, 'خطأ في التحديث'));
        });
      }
    }
    ...
    console.error('❌ خطأ عام في تحديث حالة الطلب:', {
      error: error,
      formattedError: JSON.stringify(error, null, 2),
      operation: 'updateOrderStoreResponse',
      orderId,
  ...
  try {
    ...
    if (error) {
      console.error('❌ خطأ في جلب تفاصيل الطلب:', {
        error: error,
        formattedError: JSON.stringify(error, null, 2),
        operation: 'getOrderWithProducts',
        orderId,
        stack: error?.stack
      });
      return {
    ...
    console.error('❌ خطأ عام في جلب تفاصيل الطلب:', {
      error: error,
      formattedError: JSON.stringify(error, null, 2),
      operation: 'getOrderWithProducts',
      orderId,
...
export async function markOrderAsCompleted(orderId: string, storeId: string): Promise<OrderStatusResult> {
  try {
    ...
    if (error) {
      console.error('❌ خطأ في تحديد الطلب كمكتمل:', {
        error: error,
        formattedError: JSON.stringify(error, null, 2),
        operation: 'markOrderAsCompleted',
        orderId,
        storeId,
        stack: error?.stack
      });
    ...
    console.error('❌ خطأ عام في تحديد الطلب كمكتمل:', {
      error: error,
      formattedError: JSON.stringify(error, null, 2),
      operation: 'markOrderAsCompleted',
      orderId,
